...
class BackendConfig(BaseModel):
    ...
    model: str
    base_url: Optional[str] = None        # override for vLLM, Ollama, OpenRouter, etc.
...
class VariantConfig(BaseModel):
    ...
    skill_context: Optional[str] = None  # auto-loaded from SKILL.md when mode=skill
    model: Optional[str] = None          # if None, inherits generation.model
    temperature: float = 0.0
...
class OutputConfig(BaseModel):
    ...
    file: Optional[str] = None           # if None, writes to stdout
...
class SkillModeConfig(BaseModel):
    ...
    positional_judge: Optional[PositionalJudgeConfig] = None
